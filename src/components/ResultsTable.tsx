        ...
        <g style={{ animation: "train-bob 0.5s ease-in-out infinite" }}>
          ...
          {[50, 90, 155, 200, 240].map((cx) => (
            ...
            >
              ...
              <line x1={cx - 6} y1={97} x2={cx + 6} y2={97} stroke="#818cf8" strokeWidth={1.5} />
              <line x1={cx} y1={91} x2={cx} y2={103} stroke="#818cf8" strokeWidth={1.5} />
              <line x1={cx - 4} y1={93} x2={cx + 4} y2={101} stroke="#818cf8" strokeWidth={1} />
              <line x1={cx + 4} y1={93} x2={cx - 4} y2={101} stroke="#818cf8" strokeWidth={1} />
...
function DelayBadge({ minutes }: { minutes: number }) {
  ...
  return (
    <span
      ...
      style={{ animation: "pop-in 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) both" }}
    >
      +{minutes} min
    </span>
  );
  ...
  if (error) {
    return (
      <div
        ...
        style={{ animation: "shake 0.35s ease-in-out, section-enter 0.3s ease-out both" }}
      >
  ...
  return (
    <div className="space-y-3" style={{ animation: "section-enter 0.4s ease-out both" }}>
      <div className="flex items-center justify-between">
        ...
        >
          Viser <span className="font-semibold">{results.length}</span>{" "}
          forsinkede tog
        </p>
        {results.length >= 1500 && (
          <span
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700 text-sm">
          <thead className="bg-slate-50 dark:bg-slate-700/50">
            <tr>
              {["Linje", "Retning", "Avgang", "Ankomst", "Forsinkelse"].map(
                (h) => (
                  <th
                    key={h}

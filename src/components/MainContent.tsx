...
export default function MainContent({ stations }: MainContentProps) {
  ...
  const [customCredentials, setCustomCredentials] = useState<Record<string, unknown> | null>(null);
  ...
  async function handleSearch(params: SearchParams) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Klarte ikke å koble til serveren",
      );
      setResults([]);
      setResultsKey((k) => k + 1);
    } finally {

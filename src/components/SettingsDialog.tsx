...
export default function SettingsDialog({
  ...
  function handleSave() {
    ...
    window.dispatchEvent(
      new StorageEvent("storage", { key: "customServiceAccount" }),
    );
    onCredentialChange();
    onClose();
  }
          ...
          {helpOpen && (
            <div className="border-t border-slate-200 dark:border-slate-700 px-4 py-4 text-sm text-slate-600 dark:text-slate-300 space-y-4">
              ...
              <div className="rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 px-3 py-2 text-xs text-amber-800 dark:text-amber-300">
                <strong>Kostnad:</strong> BigQuery sin gratiskvote inkluderer 1
                TB med spørringer per måned (data som prosesseres, ikke
                returneres). Hvert søk i denne appen bruker typisk 1–100 GB, noe
                som gir 10-1000 søk gratis.
              </div>
            </div>
          )}
        </div>
